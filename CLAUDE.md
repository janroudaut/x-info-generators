...
| `index.py` | `--index` catalog: scans already-generated pages (`parse_page` reads `<title>` suffix, `img.header-image`, `.ratings`, `.details-table`, and for videos the Director(s) row + `.cast-list` names for the search box), renders `index.html.j2` via `build_catalog()`. Couples to the content templates' structure. |
...
- Video stills: `video/processing.py::_resolve_screenshots()` picks the source per `--screenshot-source` — under `auto` ffmpeg frames from the local file come first, TMDB backdrops (`fetch_tmdb_stills`) only when there is no file or it yields nothing. `online`/`ffmpeg`/`off` force the behaviour. Online stills are what makes a page possible from a name alone.
...
| FFmpeg | `ffmpeg-python` for screenshot extraction — the **default** source under `auto`, TMDB backdrops taking over when there is no local file (`--screenshot-source` controls this; optional, skipped with a warning if not in PATH). `ffprobe` also fills the file-details section (resolution, audio/subtitle tracks with flags via `utils.lang_flag`; per-episode for series, union in the series details) |
